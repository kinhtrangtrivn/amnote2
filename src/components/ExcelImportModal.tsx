...
const COLUMN_MAPPINGS: ColumnMapping[] = [
  ...
  { softwareColumn: 'parentObject', excelColumn: '', required: false, description: 'ID đối tượng gốc - số nguyên (tùy chọn, để trống nếu không có cha)' },
...
export default function ExcelImportModal({ isOpen, onClose, onImport, existingData = [] }: ExcelImportModalProps) {
  ...
  const loadColumnsFromSheet = useCallback((wb: XLSX.WorkBook, sheetName: string, headerRowNum: number) => {
    try {
  ...
  const handleDownloadTemplate = useCallback(() => {
    const headers = ['Mã đối tượng*', 'Tên tiếng Việt*', 'Tên tiếng Anh', 'Tên tiếng Hàn', 'ID đối tượng gốc', 'Ghi chú'];
    const sampleData = [
      ...
      ['CC003', 'Phòng Kế Toán', 'Accounting Dept', '회계부', '', 'Bộ phận kế toán'],
    ...
    const instructionsData = [
      ...
      ['- Để trống: đối tượng không có cha (hợp lệ)'],
      ['- Nhập số: ID của đối tượng cha (phải tồn tại trong hệ thống)'],
      ['- Không được nhập chữ'],
      [''],
      ['Ví dụ:'],
      ['- CC001: không có cha (để trống cột ID đối tượng gốc)'],
  ...
  const validateData = useCallback((data: ExcelData[]): ValidationResult[] => {
    ...
    return data.map((row, index) => {
      ...
      if (mappedData.code && String(mappedData.code).trim()) {
        ...
        const duplicateIndex = data.findIndex((otherRow, otherIndex) => {
          ...
          return String(otherCode).trim() === String(mappedData.code).trim();
        });
        
        if (duplicateIndex !== -1) {
          errors.push(`Mã đối tượng "${mappedData.code}" bị trùng lặp trong file (dòng ${duplicateIndex + headerRow + 1})`);
        }

        // Check for duplicate codes with existing data
        const existingCodeDuplicate = existingData.find(existing => 
          existing.code === String(mappedData.code).trim()
        );
        if (existingCodeDuplicate) {
          errors.push(`Mã đối tượng "${mappedData.code}" đã tồn tại trong hệ thống`);
        }
      }
      ...
      console.log(`Row ${result.rowIndex}:`, result);
      return result;
    });
  }, [columnMappings, headerRow, existingIds, existingData]);
  ...
  const handleSelectRows = useCallback((type: 'all' | 'valid' | 'invalid') => {
    setFilterType(type);
    switch (type) {
      case 'all':
                  ...
                  {idx < arr.length - 1 && (
                    <div
                      className={`flex-1 h-1 mx-2 ${
                        currentStep > step ? 'bg-green-300' : 'bg-gray-300'
                      }`}
                    />
                  )}
                </React.Fragment>
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">
                      ...
                      <div className="text-xs text-gray-500 mt-1">
                        Nhập số dòng chứa tiêu đề cột (ví dụ: 1)
                      </div>
                    </label>
                    <input
                      type="number"
                  ...
                  <div>
                    ...
                    <div className="space-y-2">
                      <label className="flex items-center">
                        ...
                        />
                        <span className="ml-2 text-sm">Thêm mới (không ghi đè dữ liệu cũ)</span>
                      </label>
                      <label className="flex items-center">
                        <input
                          type="radio"
                          ...
                          className="text-green-600 focus:ring-green-500"
                        />
                        <span className="ml-2 text-sm">Cập nhật (chỉ sửa bản ghi đã tồn tại)</span>
                      </label>
                      <label className="flex items-center">
                        <input
                          type="radio"
                          ...
                          className="text-green-600 focus:ring-green-500"
                        />
                        <span className="ml-2 text-sm">Ghi đè (xóa tất cả và thêm mới)</span>
                      </label>
                    </div>
                  </div>
                  ...
                  {existingData.length > 0 && (
                    <div className="bg-amber-50 border border-amber-200 rounded-lg p-3">
                      ...
                      <div className="text-xs text-amber-800">
                        <p>Tổng số đối tượng: <strong>{existingData.length}</strong></p>
                        <p>ID có sẵn: {Array.from(existingIds).slice(0, 10).join(', ')}{existingIds.size > 10 ? '...' : ''}</p>
                      </div>
                    </div>
                  )}
                </div>
                          ...
                          <div className="break-words">
                            ...
                            {mapping.softwareColumn === 'parentObject' && (
                              <div className="mt-1 text-xs text-blue-600">
                                <strong>Lưu ý:</strong> Để trống = không có cha, nhập số = ID đối tượng cha
                              </div>
                            )}
                          </div>
                        </td>
                ...
                <div className="flex items-start space-x-2">
                  ...
                  <div className="text-sm text-blue-800">
                    ...
                    <ul className="list-disc list-inside space-y-1">
                      <li><strong>Để trống:</strong> Đối tượng không có cha (hợp lệ)</li>
                      <li><strong>Nhập số:</strong> ID của đối tượng cha (phải tồn tại trong hệ thống)</li>
                      <li><strong>Không được nhập chữ</strong> - chỉ chấp nhận số nguyên</li>
                      <li>Hệ thống sẽ kiểm tra ID có tồn tại trong danh sách {existingData.length} đối tượng hiện có</li>
                    </ul>
                  </div>
                </div>
              </div>
              ...
              <div className="mb-6">
                ...
                {process.env.NODE_ENV === 'development' && (
                  <div className="mb-4 p-3 bg-yellow-50 border border-yellow-200 rounded-lg text-xs">
                    <p><strong>Debug:</strong> Tổng số dòng validation: {validationResults.length}</p>
                    <p><strong>Excel data:</strong> {excelData.length} dòng</p>
                    <p><strong>Existing IDs:</strong> {Array.from(existingIds).join(', ')}</p>
                  </div>
                )}
                    ...
                    <table className="w-full table-fixed">
                      ...
                      <thead className="bg-gray-50 sticky top-0">
                        <tr>
                          ...
                          <th className="px-4 py-4 text-left text-sm font-medium text-gray-700 whitespace-nowrap">Dòng</th>
                          <th className="px-4 py-4 text-left text-sm font-medium text-gray-700 whitespace-nowrap">Trạng thái</th>
                          <th className="px-4 py-4 text-left text-sm font-medium text-gray-700 whitespace-nowrap">Mã</th>
                          <th className="px-4 py-4 text-left text-sm font-medium text-gray-700 whitespace-nowrap">Tên Việt</th>
                          <th className="px-4 py-4 text-left text-sm font-medium text-gray-700 whitespace-nowrap">Tên Anh</th>
                          <th className="px-4 py-4 text-left text-sm font-medium text-gray-700 whitespace-nowrap">ID đối tượng gốc</th>
                          <th className="px-4 py-4 text-left text-sm font-medium text-gray-700 whitespace-nowrap">Ghi chú</th>
                          <th className="px-4 py-4 text-left text-sm font-medium text-gray-700 whitespace-nowrap">Chi tiết lỗi</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-gray-200">
                        {getFilteredResults.map((result) => (
                          <tr key={result.rowIndex} className="hover:bg-gray-50">
                              ...
                              <div className="flex items-center space-x-2">
                                {result.isValid ? (
                                  <>
                                    ...
                                    <span className="text-sm text-green-700 whitespace-nowrap">Hợp lệ</span>
                                  </>
                                ) : (
                                  <>
                            ...
                            <td className="px-4 py-4 text-sm text-gray-900 truncate" title={result.data.code || ''}>{result.data.code || ''}</td>
                            <td className="px-4 py-4 text-sm text-gray-900 truncate" title={result.data.nameVi || ''}>{result.data.nameVi || ''}</td>
                            <td className="px-4 py-4 text-sm text-gray-900 truncate" title={result.data.nameEn || ''}>{result.data.nameEn || ''}</td>
                            <td className="px-4 py-4 text-sm text-gray-900 truncate" title={result.data.parentObject || ''}>
                              {result.data.parentObject || <span className="text-gray-400 italic">trống</span>}
                            </td>
                            <td className="px-4 py-4 text-sm text-gray-900 truncate" title={result.data.notes || ''}>{result.data.notes || ''}</td>
                            <td className="px-4 py-4">
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-4">
              ...
              {currentStep < 3 ? (
                ...
                >
                  <Play size={16} />
                  <span className="hidden sm:block"> Thực hiện ({selectedRows.filter(r => validationResults.find(v => v.rowIndex === r)?.isValid).length})</span>
                </button>
              )}
            </div>
          </div>
